...
class RoleTaskConstraints(PromptTechnique):
    ...
    def build_messages(self, evidence: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        """
        taxa_str = ", ".join(
            [f"{t[0]} ({t[1]:.2f})" for t in evidence.get("known_taxa", [])]
        )
...
class SelfConsistency(PromptTechnique):
    ...
    def __init__(
        self,
        base_technique: Optional[PromptTechnique] = None,
        n_samples: int = 3,
    ...
    def run(self, evidence: Dict[str, Any], temperature: float = 0.7) -> PromptResult:
        ...
        for taxon, confidences in pathogen_votes.items():
            if len(confidences) > majority_threshold:
                consensus_pathogens.append(
                    {"taxon": taxon, "confidence": statistics.median(confidences)}
                )
        ...
        consensus_result: Dict[str, Any] = {
            ...
            "caveats": [
                f"Consensus from {len(sub_results)} samples",
                "Multiple sampling analysis",
            ],
        }
        ...
        result["valid"] = True
        result["latency_s"] = sum(
            sr.get("latency_s", 0) for sr in sub_results if isinstance(sr, dict)
        )
...
class CritiqueAndRevise(PromptTechnique):
    ...
    def run(self, evidence: Dict[str, Any], temperature: float = 0.3) -> PromptResult:
        ...
        try:
            ...
            initial_response = self.client.chat(
                initial_messages, temperature=temperature
            )

            # Stage 2: Critique and revise
            critique_messages = self._build_critique_messages(
                initial_response["content"], evidence
            )
            final_response = self.client.chat(
                critique_messages, temperature=temperature
            )
            ...
            result["latency_s"] = (
                initial_response["latency_s"] + final_response["latency_s"]
            )
    ...
    def _build_critique_messages(
        self, initial_analysis: str, evidence: Dict[str, Any]
    ) -> List[Dict[str, str]]:
